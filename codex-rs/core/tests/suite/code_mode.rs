...
#[cfg_attr(windows, ignore = "no exec_command on Windows")]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn code_mode_terminate_cancels_in_flight_nested_tool() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let test = builder.build(&server).await?;
    let started_file = test.workspace_path("code-mode-nested-tool-started");
    let done_file = test.workspace_path("code-mode-nested-tool-done");
    let started_file_quoted = shlex::try_join([started_file.to_string_lossy().as_ref()])?;
    let done_file_quoted = shlex::try_join([done_file.to_string_lossy().as_ref()])?;
    let long_command = format!(
        "printf started > {started_file_quoted}; sleep 2; printf done > {done_file_quoted}"
    );
    let wait_command =
        format!("while [ ! -f {started_file_quoted} ]; do sleep 0.01; done; printf ready");
    let code = format!(
        r#"
text("before yield");
yield_control();
await tools.exec_command({{ cmd: {long_command:?} }});
text("after nested tool");
"#
    );
    ...
    test.submit_turn("start yielded exec with nested tool")
        .await?;
    ...
    let second_completion = responses::mount_sse_once(
        &server,
        sse(vec![
            ev_assistant_message("msg-2", "nested tool started"),
            ev_completed("resp-4"),
        ]),
    )
    .await;

    test.submit_turn("wait for nested tool to start").await?;
    ...
    let third_completion = responses::mount_sse_once(
        &server,
        sse(vec![
            ev_assistant_message("msg-3", "terminated"),
            ev_completed("resp-6"),
        ]),
    )
    .await;

    test.submit_turn("terminate the code mode cell").await?;
    ...
    assert!(
        !done_file.exists(),
        "nested exec_command continued after code-mode cell termination"
    );
