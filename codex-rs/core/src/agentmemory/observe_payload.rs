...
fn canonical_pre_tool_use(payload: Map<String, Value>) -> Result<Value, String> {
    ...
    let tool_input = payload
        ...
        .ok_or_else(|| "pre_tool_use payload missing `tool_input`".to_string())?;
    let (tool_name, tool_input) = normalize_tool_identity_and_input(&tool_name, tool_input);
...
fn normalize_tool_identity_and_input(tool_name: &str, tool_input: &Value) -> (String, Value) {
    if tool_name == "apply_patch"
        && let Some(command) = tool_input.get("command").and_then(Value::as_str)
    {
        return (
            apply_patch_memory_tool_name(command).to_string(),
            json!({
                "patch": command,
    ...
    (
        tool_name.to_string(),
        normalize_tool_input(tool_name, tool_input),
    )
}

fn normalize_tool_input(tool_name: &str, tool_input: &Value) -> Value {
    match tool_input {
...
fn apply_patch_memory_tool_name(command: &str) -> &'static str {
