...
| `patch` | string | yes | Hashline ops such as `SWAP 12:ab:`, `SWAP 12:ab..14:cd:`, a `[path#HASH]` section plus ops, multiple `[path#HASH]` sections for existing-file multi-file edits, or `[path]` sections with `create=true` for missing-file creation. Payload rows may use README-style `+` prefixes, bare replacement lines, or uniform pasted read-output rows like `1:ab\|content`; `++` emits a literal `+`, `+-` emits a literal `-`, and `+1:ab\|content` emits the read-output prefix literally. Sectioned patches also accept reference-style `REM` and `MV <path>` file ops; `*** Abort` suppresses an embedded patch without writing. |
