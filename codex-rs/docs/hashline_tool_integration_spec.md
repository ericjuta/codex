...
| Patch grammar | `/tmp/hashline/crates/core/src/tokenizer.rs`, `/tmp/hashline/crates/core/src/parser.rs` | Supports `SWAP`, `DEL`, `INS.PRE`, `INS.POST`, `INS.HEAD`, `INS.TAIL`, block operations (`SWAP.BLK`, `DEL.BLK`, `INS.BLK.POST`, `INS.BLK.PRE`, `INS.BLK`), and sectioned `REM`/`MV`. |
...
| `patch` | string | yes | Hashline ops such as `SWAP 12:ab:`, a `[path#HASH]` section plus ops, or multiple `[path#HASH]` sections for existing-file multi-file edits. Sectioned patches also accept reference-style `REM` and `MV <path>` file ops. |
